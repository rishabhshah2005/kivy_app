...
class MsgWindow(Screen):
    def create_popup(self, title, msg: str):
        popup = Popup(title=title,
            content=Label(text=msg),
            size_hint=(None, None), size=(400, 400)
            )
        popup.open()
        Clock.schedule_interval(lambda x: popup.dismiss(), 3)
    ...
    def __init__(self, sm, s, self_name, **kwargs):
        ...
        self.recv_thread = threading.Thread(target=recieve, args=(self.s, self.msgs, self.thread_event))
...
class MainScreen(Screen):
    def create_popup(self, title, msg: str, dur=3):
        popup = Popup(title=title,
            content=Label(text=msg),
